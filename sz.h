...
{
 public:
  Sz(string name_in,int q_in,StateSpace& statespace_in);
...
Sz::Sz(string name_in,int q_in,StateSpace& statespace_in):name(name_in),q(q_in),statespace(statespace_in),N(statespace_in.GetN()),Operator(name_in,statespace_in.Nsectors())
{
  if(WTRACE) cout << "Constructing the Sz(" << q << ") operator" << endl;
  ...
  for(int is1=0; is1<Nsectors; is1++)
    {
      ...
      int k2=(k1+q)%N;     // Sz increases the momentum by q
...
void Sz::GenerateSector(const int is1)
{
  ...
  if(WTRACE) cout << "Constructing Sz(" << q << ") for sector: " << is1 << endl;
  logfile << "\nConstructing Sz(" << q << ") for sector: " << is1 << endl;
  ...
  for(int i=startstate+1; i<Nstates1; i++)
    {
      ...
      for(int r=0; r<N; r++)
	{
	  double m=statespace.Sz(s,r);    // 0.5 if up 
	  j    = sec2.FindIndx(s); 
	  if(j<Nstates2)
	    {
	      jn   = sec2.state[j].norm; 
	      A.AddTo(j,i,g[r]*sqrt(jn/in)*m);
	    }
...
disk" << endl;
  ...
  if(WTRACE) cout << "Done constructing Sz(" << q << ") for sector # " << sec1.id << endl;
  logfile << "Done constructing Sz(" << q << ") for sector # " << sec1.id << endl;
