...
class Stateref
{
  friend ostream& operator<<(ostream& os,const Stateref& r){os << "(" << bitstring(r.s,2)
 << "," << r.norm << ")"; return os;}
public:
...
private:
  ...
  pstate Spininversion(pstate s,int n){return  (n==0 ? s : flpbits(s,0,N));}

  int Nspinups(pstate s){return bitcount(s);}
  int Ndwalls(pstate s){return Nbitwalls(s,N);}
  int SpinParity(pstate s){return Nspinups(s)%2;}

};

StateSpace::StateSpace(const int n_in,const int ndw_in):N(n_in),Ndw(ndw_in),zerostate(pstate0),maxstate(setbits(pstate0,0,n_in)),Nconsvalues(MAXCONSLAWS), Ntotsectors(1)
{
	...
	for(int i=0; i<list.size(); i++)
	  {
		...
		//cout << "state: " << state << " norm:" << norm << endl;
		if(norm !=0)
		  {
...
vector<pstate> StateSpace::Negate(const vector<pstate> inlist)
{
  ...
  for(int item=0; item<inlist.size(); item++)
    {
      tlist.push_back(flpbits(inlist[item],0,N));
    }
  return tlist;
}
