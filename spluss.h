...
class Spluss : public Operator
{
 public:
  Spluss(string name_in,int q_in,StateSpace& statespace_in);
...
Spluss::Spluss(string name_in,int q_in,StateSpace& statespace_in):name(name_in),q(q_in),statespace(statespace_in),N(statespace_in.GetN()),Operator(name_in,statespace_in.Nsectors())
{
  if(WTRACE) cout << "Constructing the operator S+(" << q << ")" << endl;
...
void Spluss::GenerateSector(const int is1)
{
  // construct the operator for sector is1
  if(WTRACE) cout << "\n***Constructing S+(" << q << ") operator for sector " << is1 << endl;
  logfile << "\n***Constructing S+(" << q << ") operator for sector " << is1 << endl;
  ...
  for(int i=startstate+1; i<Nstates1; i++)
      ...
      for(int r=0; r<N; r++)
	{
	  ...
	  // go thru all terms in the operator
	  ...
	  if(j<Nstates2)
	    {
	      ...
	      A.AddTo(j,i,f[r]*g[ri]*sqrt(jn/in));
...
disk" << endl;
  ...
  if(TRACE){
    cout << "------Spluss in sector " << is1 << " is------" << endl;
    cout << A << endl;
    cout << "---------------" << endl;
  }
  
  if(TRACE) cout << "The matrix Spluss is " << (A.IsReal() ? "real": "complex") << endl;
  ...
  if(WTRACE) cout << "Done constructing the S+(" << q << ") for sector # " << sec1.id << endl;
  logfile << "Done constructing the S+(" << q << ") for sector # " << sec1.id << endl;
