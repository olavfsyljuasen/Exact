...
const double LANCZOSEXITLIMIT = 1.e-15; // should not be too small, otherwise there will be non-orthogonality
...
Lanczos_impl::Lanczos_impl(MyMatrix& H_in,int M_in,int basestatenr):H(H_in),N(H.Ncols()),M(min(N,M_in)),Mstop(M),Niterations(0),phi(NULL),lanczos_a(M),lanczos_b(M-1),LM(0),initnorm(1.)
  {
	...
	if(TRACE)
	  {
	    cout << "phi[0]:" << endl;
	    for(int i=0; i<N; i++) cout << setprecision(DEBUGPRECISION) << phi0[i] << endl;
	  }
...
void Lanczos_impl::CheckOrthogonality(int N,MyField* z,const int m)
{
  if(WTRACE) cout << "Checking orthogonality:" << endl;
  for(int i=0; i<=m; i++)
    {
      MyField* phii=&phi[i*N];
      MyField q=ScalarProduct(&N,z,phii); // q=<z|phi_i>
      if(TRACE)
	if(abs(q) > 1.e-13)
	  { 
	    cout << " WARNING: Severe Loss of Orthogonality!";
	    cout << " <z|phi[" << i << "]>=" << setprecision(DEBUGPRECISION) << q;
	    cout << endl;
	  }
    }
}
...
void Lanczos_impl::Reorthogonalize(int N,MyField* z,const int m)
  {
    ...
    // MGSREPEATSTEPS=1: Modified Gram-Schmidt
    // MGSREPEATSTEPS=2: Modified Gram-Schmidt with reorthogonalization

    for(int j=0; j<MGSREPEATSTEPS; j++) 
      for(int i=0; i<=m; i++) // Modified Gram-Schmidt 
	{
	  MyField* phii=&phi[i*N];
	  MyField negq=-ScalarProduct(&N,phii,z); // q=<phi_i|z>
...
bool Lanczos_impl::LanczosIteration(const int m)
  {
    ...
    if(TRACE) 
      {
	cout << "H * phi[" << m << "]" << endl;
	for(int i=0; i<N; i++){cout << setprecision(DEBUGPRECISION) << z[i] << endl;}
      }
    ...
    if(TRACE) CheckOrthogonality(N,z,m);
    ...
    // stop the Lanczos procedure; 
    if(abs(lanczos_b[m])<=LANCZOSEXITLIMIT){return false;} 
    //    else{CheckOrthogonality(N,z,m);}
    ...
    Reorthogonalize(N,z,m);
    */
    if(TRACE) cout << "After Normalizing:" << endl;
    if(TRACE) CheckOrthogonality(N,z,m);
	...
	if(TRACE) 
	  {
	    cout << "phi1:" << endl;
	    for(int i=0; i<N; i++){cout << setprecision(DEBUGPRECISION) << phi1[i] << endl;}
	  }
    ...
    if(TRACE){ 
      ...
      MyField* v=&phi[m*N];
      for(int i=0; i<N; i++){cout << setprecision(DEBUGPRECISION) << v[i] << endl;}

      cout << "a[" << m << "]=" << setprecision(DEBUGPRECISION) << lanczos_a[m];
      if(m<M-1) cout << " b[" << m << "]=" << setprecision(DEBUGPRECISION) << lanczos_b[m];
      cout << endl;
    }
