...
class Sx : public Operator
{
 public:
  Sx(string name_in,int q_in,StateSpace& statespace_in);
...
Sx::Sx(string name_in,int q_in,StateSpace& statespace_in):name(name_in),q(q_in),statespace(statespace_in),N(statespace_in.GetN()),Operator(name_in,statespace_in.Nsectors())
{
  if(WTRACE) cout << "Constructing the operator Sx(" << q << ")" << endl;
  ...
  for(int is1=0; is1<Nsectors; is1++)
    {
      ...
      int k2=(k1+q)%N;     // Sx increases the momentum by q
      ...
      cout << "in sx.h: Sx does not conserve magnetization!" << endl;
      exit(1);
...
void Sx::GenerateSector(const int is1)
{
  // construct the operator for sector is1
  if(WTRACE) cout << "\n***Constructing Sx(" << q << ") operator for sector " << is1 << endl;
  logfile << "\n***Constructing Sx(" << q << ") operator for sector " << is1 << endl;
	  ...
	  if(j<Nstates2)
	    {
	      ...
	      A.AddTo(j,i,0.5*f[r]*g[ri]*sqrt(jn/in));
...
disk" << endl;
  ...
  if(TRACE){
    cout << "------Sx in sector " << is1 << " is------" << endl;
    cout << A << endl;
    cout << "---------------" << endl;
  }
  
  if(TRACE) cout << "The matrix Sx is " << (A.IsReal() ? "real": "complex") << endl;
  ...
  if(WTRACE) cout << "Done constructing the Sx(" << q << ") for sector # " << sec1.id << endl;
  logfile << "Done constructing the Sx(" << q << ") for sector # " << sec1.id << endl;
